...
function saveBooking(bookingData) {
  ...
  if (!bookingData || !bookingData.day || !bookingData.startTime || !bookingData.endTime) {
    return { success: false, message: "بيانات الحجز غير مكتملة" };
  }
  
  var allBookings = getAllBookings();
  var conflict = checkBookingConflict(bookingData, allBookings);
  ...
  if (bookingData.id) {
    ...
    var newId = lastRow;
    sheet.getRange(lastRow + 1, 1).setValue(newId);
    sheet.getRange(lastRow + 1, 2).setValue(safeInt(bookingData.hallId, ''));
    sheet.getRange(lastRow + 1, 3).setValue(safeInt(bookingData.trainerId, ''));
    sheet.getRange(lastRow + 1, 4).setValue(safeInt(bookingData.groupId, ''));
    ...
    sheet.getRange(lastRow + 1, 8).setValue(safeInt(bookingData.createdBy, 1));
